...
/**
 * LoginHandler
 * Processes authentication requests, profile registrations, and session instantiation.
 */
class LoginHandler {
    /**
     * @param {Server} server 
    ...
    static register(server, db, dataLoader, worldMaps) {
        ...
        server.registerHandler(PacketIDs.LOGIN_REQUEST, (session, payload) => {
            ...
            const chars = db.statements.getCharactersByAccount.all(account.id);
        ...
        server.registerHandler(PacketIDs.CHAR_SELECT, (session, payload) => {
            ...
            // CRITICAL NOTE FIX: Update entry player with all current objects inside the active cell block
